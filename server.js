...
app.put('/api/pos/send/:hn', (req, res) => {
  ...
  // 🌟 แก้ไขให้ค้นหาคิวที่ "กำลังตรวจอยู่ (CHECKED_IN)" ของคนไข้คนนี้โดยตรง
  db.get(`SELECT id FROM appointments WHERE patient_id = ? AND status = 'CHECKED_IN' ORDER BY id DESC LIMIT 1`, [hn], (err, row) => {
    if (err) return res.status(500).json({ status: 'error', message: err.message });

    if (row) {
      // เปลี่ยนสถานะคิวใบเดิมให้เป็น "รอชำระเงิน"
      ...
      // ถ้าไม่เจอ แสดงว่าถูกส่งไปแล้ว ป้องกันการกดเบิ้ล
      res.json({ status: 'error', message: 'ไม่พบคิวที่กำลังตรวจ (คิวอาจถูกส่งไปแล้ว)' });
    }
  });
});
